...
function getRegionsSuccess(response) { // getCircumferenceRangeSuccess;
  const regionsInDropdown = [{ id: 'Alla', label: 'Alla' }];
  $.each(response.features, (index, value) => {
    const obj = { id: value.attributes.Kommun, label: value.attributes.Kommun };
    regionsInDropdown.push(obj);
  });
  createSelect('.filterSelect.region-select', regionsInDropdown);
}
